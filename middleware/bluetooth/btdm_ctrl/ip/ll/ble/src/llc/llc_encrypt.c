...
enum llc_encrypt_state
{
    ...
    /// Wait for REQ_ENC_RSP_ACK
    LLC_REM_WAIT_REQ_ENC_RSP_ACK,
...
__STATIC void llc_ll_request_enc_rsp_ack_handler(uint8_t link_id, uint8_t op_code);
/*
...
__STATIC void llc_loc_encrypt_proc_continue(uint8_t link_id, uint8_t state, uint8_t status)
{
    FUNC_PATCH_ENTRY_3_PARAM_NO_RETURN(PROC_CONTINUE_PATCH_TYPE, LLC_LOC_ENCRYPT_PROC_CONTINUE_C_FUN_BIT, link_id, state, status);
    ...
    else
    {
        ...
        * @enduml
        ...
        default:
        {
            ...
            ASSERT_INFO_FORCE(0, link_id, proc_state);
        }
        break;
        }
    }
...
__STATIC void llc_rem_encrypt_proc_continue(uint8_t link_id, uint8_t state, uint8_t status)
{
    FUNC_PATCH_ENTRY_3_PARAM_NO_RETURN(PROC_CONTINUE_PATCH_TYPE, LLC_REM_ENCRYPT_PROC_CONTINUE_C_FUN_BIT, link_id, state, status);
    /// Gets the LLC environment dedicated to this link
    struct llc_env_tag *llc_env_ptr = llc_env[link_id];
    /// retrieve procedure parameters
    ...
    else
    {
        ...
        case LLC_REM_WAIT_IVS_SKDS_GEN:
        {
            ...
            if (llm_le_evt_mask_check(LE_EVT_MASK_LG_TR_KEY_REQ_EVT_BIT))
            {
                ...
                // send LTK Request
                llc_proc_state_set(&param->proc, link_id, LLC_REM_WAIT_REQ_ENC_RSP_ACK);
        ...
        break;
        case LLC_REM_WAIT_REQ_ENC_RSP_ACK:
        {
...
__STATIC void llc_ll_enc_rsp_pdu_send(uint8_t link_id, uint8_t *skds, uint8_t *ivs)
{
    ...
    llc_llcp_send(link_id, (union llcp_pdu *) &pdu, llc_ll_request_enc_rsp_ack_handler);
}
...
/**
 ****************************************************************************************
 * @brief Callback called when LL_ENC_RSP_OPCODE Tx Ack is received
 *
...
__STATIC void llc_ll_request_enc_rsp_ack_handler(uint8_t link_id, uint8_t op_code)
{
    ...
    {
        llc_rem_encrypt_proc_continue(link_id, LLC_REM_WAIT_REQ_ENC_RSP_ACK, CO_ERROR_NO_ERROR);
    }
}
