...
typedef void (*rf_para_configure_handler)(void);
...
rf_init_handler _rf_init_handler = _rf_init;
rf_para_configure_handler _rf_para_configure_handler = rf_para_configure;
...
{
//Yier, sleep on still use MAC register.
#if 0
...
#else
#ifdef SOC_BF_Z0
    ip_aon_slp_ctrl_sleep_req_setf(1);
    // Wait MAC enter sleep, ~30us
    while (ip_aon_slp_ctrl_sleep_status_getf() == 0);
#else
    // TODO: Implement for A0
...
static void rf_para_configure(void)
{
...
#ifdef FPGA
    ble_radiocntl0_set(0x0A140A00);
#else
    ble_radiocntl0_set(0x1A140A00);
#endif
    /* BLE RADIOCNTL3 */
    ble_radiocntl3_pack(/*uint8_t rxrate3cfg*/    0x3, // map on 1 Mbps
            /*uint8_t rxrate2cfg*/    0x2, // map on 1 Mbps
            /*uint8_t rxrate1cfg*/    0x1,
            /*uint8_t rxrate0cfg*/    0x0,
            /*uint8_t rxsyncrouting*/ 0x0,
            /*uint8_t rxvalidbeh*/    0x0,
            /*uint8_t txrate3cfg*/    0x3, // map on 1 Mbps
            /*uint8_t txrate2cfg*/    0x2, // map on 1 Mbps
    ...
    /* BLE RADIOTXRXTIM0 */
    ble_radiotxrxtim0_pack(/*uint8_t rfrxtmda0*/   0x18,
            /*uint8_t rxpathdly0*/  0x18,
            /*uint8_t txpathdly0*/  0x07);
 ...
 ***************************************************************************************/
void _rf_init(struct rwip_rf_api *api)
{
#if defined(CFG_BT)
    ...
    /* begin: temp for diag */
    //*(volatile uint32_t *)0x41020050 = 0x8383;
    /* end: temp for diag*/
