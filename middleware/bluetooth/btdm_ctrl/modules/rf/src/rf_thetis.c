...
* @file rf_thetis.c
*
* @brief Thetis radio initialization and specific functions
*
...
/*****************************************************************************************
* @addtogroup RF_THETIS
* @ingroup RF
* @brief Thetis Radio Driver
*
...
__STATIC uint32_t rf_rpl_reg_rd(uint16_t addr);
__STATIC void rf_rpl_reg_wr(uint16_t addr, uint32_t value);

#define REG_RPL_RD                rf_rpl_reg_rd
#define REG_RPL_WR                rf_rpl_reg_wr

#include "reg_thetis.h"          // thetis register
...
{
#if defined(CFG_BT_EMB)
    ip_radiocntl0_forceagc_en_setf(en);
#else
...
void rf_init(struct rwip_rf_api *api)
{
...
#if !defined(CFG_BT_EMB)
    /* IP RADIOCNTL0 */
    ip_radiocntl0_pack(/*uint16_t spiptr*/   EM_RF_SW_SPI_OFFSET,
            /*uint8_t  spicfg*/   0,
            /*uint8_t  spifreq*/  1,
            /*uint8_t  spigo*/    0);
    ...
    ble_radiocntl2_pack(/*uint8_t  lrsynccompmode*/ 0x0,
            ...
            /*uint8_t  lrsyncerr*/      0,
            /*uint8_t  syncerr*/        0,
            /*uint16_t freqtableptr*/   EM_FT_OFFSET);
    ...
    /* IP RADIOCNTL0 */
    ip_radiocntl0_sync_pulse_mode_setf(0);
    ip_radiocntl0_dpcorr_en_setf(1);
    ip_radiocntl0_spifreq_setf(0); //6.5MHz
    ip_radiocntl0_forceagc_en_setf(1);
    ip_radiocntl0_spigo_setf(0); //spi comp can't be W, only read - should be 0

    /* BT RADIOCNTL3 */
    bt_radiocntl3_freqtable_ptr_setf(EM_FT_OFFSET >> 2);
    ...
    /* IP RADIOCNTL1 */
    ip_radiocntl1_xrfsel_setf(0x01);
    ip_radiocntl1_spiptr_setf(EM_RF_SW_SPI_OFFSET);
...
#if defined(CFG_BT_EMB)
    ip_radiocntl0_dpcorr_en_setf(0x1);
    ASSERT_ERR(ip_radiocntl0_dpcorr_en_getf() == 0x1);
#endif // CFG_BT_EMB
