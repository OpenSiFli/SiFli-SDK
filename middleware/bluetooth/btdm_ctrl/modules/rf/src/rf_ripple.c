...
#if (PLF_GAIA)
    #define RPL_LRSYNCCOMPMODE     0x3
    #define RPL_SPI_FREQ           0x1  // 16Mhz BB // 0x0  : for 8MHz BB
    #define RPL_SYNC_POS           0x21 // 16Mhz BB // 0x10 : for 8MHz BB
#else // (!PLF_GAIA)
    #define RPL_LRSYNCCOMPMODE     0x0
    #define RPL_SPI_FREQ           0x1  // 26MHz BB
    #define RPL_SYNC_POS           0x38 // 26MHz BB
#endif // !(PLF_GAIA)
...
void rf_init(struct rwip_rf_api *api)
{
...
#if defined(CFG_BLE_EMB)
    /* BLE RADIOCNTL2 */
    ble_radiocntl2_pack(/*uint8_t  lrsynccompmode*/ RPL_LRSYNCCOMPMODE,
    ...
    ble_radiotxrxtim0_pack(/*uint8_t rfrxtmda0*/   0,
            ...
            /*uint8_t txpathdly0*/  0x3 + 3 * PLF_GAIA);

    /* BLE RADIOTXRXTIM2 */
    ble_radiotxrxtim2_pack(/*uint8_t rxflushpathdly2*/ 0x10 + 3 * PLF_GAIA,
            /*uint8_t rfrxtmda2*/       0x00,
            /*uint8_t rxpathdly2*/      0x49 + 1 * PLF_GAIA,
            /*uint8_t txpathdly2*/      0x03 + 3 * PLF_GAIA);

    /* BLE RADIOTXRXTIM3 */
    ble_radiotxrxtim3_pack(/*uint8_t rxflushpathdly3*/ 0x10 + 3 * PLF_GAIA,
            /*uint8_t rfrxtmda3*/       0x00,
            /*uint8_t txpathdly3*/      0x03 + 3 * PLF_GAIA);
...
#define RPL_RX_PATH_DLY (RPL_RADIO_SKEW - RPL_TX_PATH_DLY)
    ...
    bt_radiotxrxtim_txpathdly_setf(RPL_TX_PATH_DLY + 3 * PLF_GAIA);
    bt_radiotxrxtim_sync_position_setf(RPL_SYNC_POS); // Default is 0x10
