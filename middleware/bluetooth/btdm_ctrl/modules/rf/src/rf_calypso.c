...
/*****************************************************************************************
* @addtogroup RF_CALYPSO
* @ingroup RF
* @brief Calypso DF Radio Driver
*
...
// Calypso register definitions and access functions
    ...
    __STATIC void rf_clp_mdm50_init(void);
...
#include "reg_modem50.h"                    // Modem50 register
/*****************************************************************************************
 * DEFINES
 ****************************************************************************************/
...
__STATIC void rf_clp_mdm50_init(void)
{
    ...
    mdm50_rx_startupdel_set(0x00010001);
    ASSERT_ERR(mdm50_rx_startupdel_get() == 0x00010001);

    mdm50_tx_startupdel_set(0x00010000);
    ASSERT_ERR(mdm50_tx_startupdel_get() == 0x00010000);

    mdm50_tx_gfskmode_set(0x00000001);
    ASSERT_ERR(mdm50_tx_gfskmode_get() == 0x00000001);

    //RXGFSK_CNTL_FO_COMP_NSYNC_DET_BT
    mdm50_rxgfsk_cntl_fo_comp_nsync_det_bt_setf(0);
    ASSERT_ERR(mdm50_rxgfsk_cntl_fo_comp_nsync_det_bt_getf() == 0);
...
{
    int8_t RssidBm;
    RssidBm = (int8_t)rssi_reg - 68; // -81 + 13;
...
void rf_init(struct rwip_rf_api *api)
{
    ...
    /* BLE RADIOTXRXTIM0 */
    ble_radiotxrxtim0_pack(/*uint8_t rfrxtmda0*/   12,
            /*uint8_t rxpathdly0*/  9,
            /*uint8_t txpathdly0*/  5);

    /* BLE RADIOTXRXTIM1 */
    ble_radiotxrxtim1_pack(/*uint8_t rfrxtmda1*/   4,
            /*uint8_t rxpathdly1*/  5,
            /*uint8_t txpathdly1*/  4);

    /* BLE RADIOTXRXTIM2 */
    ble_radiotxrxtim2_pack(/*uint8_t rxflushpathdly2*/ 25,
            /*uint8_t rfrxtmda2*/       139,
            /*uint8_t rxpathdly2*/      48,
            /*uint8_t txpathdly2*/      6);

    /* BLE RADIOTXRXTIM3 */
    ble_radiotxrxtim3_pack(/*uint8_t rxflushpathdly3*/ 25,
            /*uint8_t rfrxtmda3*/       43,
            /*uint8_t txpathdly3*/      6);
    ...
    /* BT RADIOTXRXTIM */
    bt_radiotxrxtim_rxpathdly_setf(9);
    bt_radiotxrxtim_txpathdly_setf(5);
...
//    ip_radiocntl1_forceiq_setf(1); TODO wait for CLP IQ calibration
    ...
    /*            -> Next PTR = 0x0000 <-> Last SPI frame
                  ...
                  -> Address of CLP_CORRELATION_CODE_BT_BLE_ADD register = 8'hD9*/
    RF_CLP_EM_WR(ble_rxonptr + 0x8,  0x0000);
    RF_CLP_EM_WR(ble_rxonptr + 0xa,  0xD983);
#endif // defined CFG_BLE_EMB
    ...
    /*            -> Next PTR = (bt_rxonptr+0xc) >> 2
                  -> Write Access of 4 bytes = 8'h83
                  -> Address of CLP_CORRELATION_CODE_BT_BLE_ADD register = 8'hD9*/
    nextptr = (bt_rxonptr + 0xc) >> 2;
    RF_CLP_EM_WR(bt_rxonptr + 0x8,  nextptr);
    RF_CLP_EM_WR(bt_rxonptr + 0xa,  0xD983);
    /*            -> Next PTR = 0x0000 <-> Last SPI frame
                  -> Write Access of 4 bytes = 8'h83
                  -> Address of CLP_CORRELATION_CODE_BT_ADD register = 8'hD8*/
    RF_CLP_EM_WR(bt_rxonptr + 0xc, 0x0000);
    RF_CLP_EM_WR(bt_rxonptr + 0xe, 0xD883);
#endif // defined CFG_BT_EMB
    ...
    RF_CLP_EM_WR(rssiptr + 0x8, 0x0000);
    RF_CLP_EM_WR(rssiptr + 0xa, 0x8700);
