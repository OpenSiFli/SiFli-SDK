...
void app_hid_init(void)
{
     ...
     */
    if (sifli_nvds_get(NVDS_TAG_MOUSE_TIMEOUT, &length, (uint8_t *)&app_hid_env.timeout) != NVDS_OK)
    {
...
void app_hid_start_mouse(void)
{
    ...
    if (sifli_nvds_get(NVDS_TAG_MOUSE_SAMPLE_RATE, &length, &rate) == NVDS_OK)
    {
...
void app_hid_enable_prf(uint8_t conidx)
{
    ...
    if (app_sec_get_bond_status())
    {
        ...
        if (sifli_nvds_get(NVDS_TAG_MOUSE_NTF_CFG, &length, (uint8_t *)&ntf_cfg) != NVDS_OK)
        {
        ...
        ke_task_id_t const src_id)
{
    ...
    if (app_hid_env.state == APP_HID_READY)
    {
        ...
        if (sifli_nvds_get(NVDS_TAG_MOUSE_ENERGY_SAFE, &length, (uint8_t *)&timer_val) != NVDS_OK)
        {
