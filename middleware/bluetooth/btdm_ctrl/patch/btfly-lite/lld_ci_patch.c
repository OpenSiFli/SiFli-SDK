...
void lld_ci_init_patch(bool is_reset)
{
    //FUNC_PATCH_ENTRY_1_PARAM_NO_RETURN(LLD_CI_2_PATCH_TYPE, LLD_CI_INIT_2_FUNC_BIT, init_type);
#if !(BLE_CIS_IN_ROM)
    if (is_reset)
    {
        // Counter
...
void lld_ci_ch_map_update_patch(uint8_t link_id, struct le_ch_map *p_ch_map, uint32_t ref_timestamp)
{
    //FUNC_PATCH_ENTRY_3_PARAM_NO_RETURN(LLD_CI_1_PATCH_TYPE, LLD_CI_CH_MAP_UPDATE_FUNC_BIT, link_id, p_map, ref_timestamp);
#if !(BLE_CIS_IN_ROM)
    uint8_t act_id;
...
#if (BLE_PERIPHERAL)
void lld_ci_sync_time_update_patch(uint8_t link_id, uint32_t last_sync_ts, int32_t sync_drift_acc)
{
