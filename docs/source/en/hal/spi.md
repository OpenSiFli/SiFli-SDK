...
Supports 4 SPI interfaces, with 2 on HCPU (SPI1, SPI2) and 2 on LCPU (SPI3,
SPI4).

## Key Features
 - Provides receive-only, transmit-only, and simultaneous transmit-receive
   modes, with each mode supporting polling, interrupt, and DMA methods

## Configurable Options
 - Master/slave
 - Shared data line for transmit/receive, or separate 2 data lines for
   transmit/receive
 - Data width: supports 8-bit/16-bit
 - Clock polarity, data sampling edge
 - Frame format: supports Motorola Serial Peripheral Interface (SPI), Texas
   Instruments Synchronous Serial Protocol (SSP), National Semiconductor
   Microwire
