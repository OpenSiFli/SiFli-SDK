...
:::{only} SF32LB57X
- **通道数量**：共 12 个通道（通道 0 ~ 11）。
- **特殊通道**：在 `SF32LB57x` 系列中，**通道 11** 固定用于电池电压检测（VBAT 经电阻分压后输入）。
:::
...
| 特性 | SF32LB55x | SF32LB56x / 58x / 52x / 57x |
| :--- | :--- | :--- |
| **采样位宽** | 10 bit | 12 bit |
| **采样精度** | 3~4 mV | 1~2 mV |
| **最大采样电压** | 1.1 V | 3.3 V |
...
![ADC分压电阻示意图](./assets/adc_resistor.png)
...
![ADC模拟电源与参考电压](./assets/adc_ana_ref.png)
...
| 通道 | GPADC_CH0 | GPADC_CH1 | GPADC_CH2 | GPADC_CH3 | GPADC_CH4 | GPADC_CH5 | GPADC_CH6 | GPADC_CH7 | GPADC_CH8 | GPADC_CH9 | GPADC_CH10 | GPADC_CH11 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
1. 输入两个准确且稳定的电压值（55x: 0.3V 和 0.8V, 56x/52x/58x/57x: 1.0V 和 2.5V）。
2. 读取对应的 ADC 寄存器值。
3. 计算斜率和偏移。
4. 保存参数供后续使用。
