...
## 支持的平台
<!-- 支持哪些板子和芯片平台 -->
+ eh-lb525,只支持连接一个耳机做音频转发，不支持通话转发
+ eh-lb563,只支持连接一个耳机做音频转发，不支持通话转发
+ eh-lb567,只支持连接一个耳机做音频转发，不支持通话转发
+ eh-lb58x
+ eh-lb57x
+ sf32lb52-lcd系列
+ sf32lb56-lcd系列，只支持连接一个耳机做音频转发，不支持通话转发
+ sf32lb58-lcd系列

## 概述
<!-- 例程简介 -->
本例程演示 A2DP 音乐分享（sharing）和 HFP 通话中继（relay）功能：设备作为中继在同时连接手机和耳机的情况下，将手机播放的音乐分享到耳机播放，同时也能将耳机端的音乐控制命令转发给手机实现音乐控制。对于支持 AVRCP 绝对音量的耳机，例程还会将手机设置的媒体音量同步到耳机。
...
例程启动后会默认使能蓝牙，蓝牙名称为‘sifli_a2dp_transfor’,可以接受手机连接或主动发起对耳机的连接。手机侧用于 A2DP sink/HFP HF 连接，耳机侧用于 A2DP source/HFP AG 连接。
...
3. 音乐分享
    ...
    7. 中继设备默认不会回连耳机和手机设备。
    8. 音量同步：同时连接支持绝对音量的手机和支持 AVRCP 绝对音量的耳机时，手机调节媒体音量后，中继设备会将音量值同步到耳机。
...
在A2DP音乐共享场景中，中继设备需要根据连接的对端设备类型动态设置AVRCP角色：

#### AVRCP角色定义
- **AVRCP TG (Target)**：音源设备，播放音乐的一方，被控制方
- **AVRCP CT (Controller)**：控制设备，发送控制命令的一方


#### 角色动态设置的必要性
在 `BT_NOTIFY_AVRCP_PROFILE_CONNECTED` 事件处理中，代码根据A2DP角色动态设置AVRCP角色：

```c
avrcp_role = (inst->con[con_idx].cfg == AV_AUDIO_SRC) ? AVRCP_TG : AVRCP_CT;
```

- 当设备作为 **A2DP Source**（连接耳机）时，设置为 **AVRCP TG**
- 当设备作为 **A2DP Sink**（连接手机）时，设置为 **AVRCP CT**

**如果错误地将所有连接固定为CT角色**，会导致以下问题：
1. 与耳机的AVRCP连接无法正确协商绝对音量功能
2. 耳机的音量变化注册请求得不到响应
3. 音量同步失败，返回 `BT_ERROR_UNSUPPORTED` 错误
4. 手机调节音量后，无法转发到耳机
