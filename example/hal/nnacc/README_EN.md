...
## Overview
NNACC (Neural Network Accelerator) is a dedicated hardware module in Sifli chips
designed to accelerate the following tasks:
...
This example verifies whether the hardware accelerator on the chip can correctly
perform Depthwise Convolution operations.
...
```
Execute `build_sf32lb58-lcd_a128r32n1_dsi_hcpu\uart_download.bat` and select the
port as prompted to download:
...
by this example (with nnacc support)
```
**Note**: 
...
msh />
```
``` Routine Log: ``` Buffers initialized successfully. testcase start.
fill_with_random_data. 2000fc60 20012808 20012894 200182bc, sp=2000e35f
HAL_NNACC_Start. Test passed: NNACC result is correct. msh />
```
## Troubleshooting
If the expected log output does not appear, check the following:
* Verify whether the hardware supports NNACC.
