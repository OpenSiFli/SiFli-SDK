...
## Overview
This example demonstrates how to dynamically load ezip format image files using
LVGL in RT-Thread environment. The example supports:
...
## Features
- **Multiple Loading Methods**: Support direct loading from file or preloading
  to memory
- **Flexible Memory Management**: Optional SRAM or PSRAM storage for image data
- **File System Support**: Support reading images from Flash file system and SD
  card
- **Command Line Control**: Dynamically switch image sources through finsh
  commands
...
- Use sdk/tools/png2ezip/ezip.exe tool to convert PNG images to ezip format
  ...
  **Command Parameter Description**:
  ...
  - `-binext .ezip`: Specify output file extension as .ezip, can be customized
    up to 20 characters
...
- Or copy the image file to the SD card root directory (SD card insertion
  required)
...
After the program starts, you can dynamically switch image sources through
...
- **LVGL V8**: Use file name directly, e.g., `example.ezip` or
  `/sdcard/image.ezip`
- **LVGL V9**: Need to add drive prefix, e.g., `A:/example.ezip` or
  `A:/sdcard/image.ezip`
...
## Notes
1. Ensure the development board has sufficient SRAM or PSRAM space to store
   image data
...
5. Preload method (dsc) has better performance but uses memory, file method
   (file) saves memory but has lower performance

## Troubleshooting
- **Image cannot be displayed**: Check if the ezip file exists and the path is
  correct
...
For other issues, please submit an
