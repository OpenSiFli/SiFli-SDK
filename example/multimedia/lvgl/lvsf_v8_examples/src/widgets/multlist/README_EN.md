...
Source path: `SiFli-SDK/example/multimedia/lvgl/lvsf_v8_examples/src/widgets/multlist`

> This example is aggregated into the `lvsf_v8_examples` project: in its `project` directory run `scons --board=<board> --menuconfig`, select **multlist** under "LVSF v8 widget examples → Select widget example", then build.
...
- Call `gui_app_init(1)` to initialize the application framework
