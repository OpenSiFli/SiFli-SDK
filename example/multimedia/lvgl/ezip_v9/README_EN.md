...
LVGL in an RT-Thread environment. The example supports:
...
## Features
- **Multiple Loading Methods**: Support direct file loading or preloading to
  memory
- **Flexible Memory Management**: Option to store image data in SRAM or PSRAM
- **Filesystem Support**: Support reading images from Flash filesystem and SD
  card
...
- Use the sdk/tools/png2ezip/ezip.exe tool to convert PNG images to ezip format
  ...
  **Command Parameter Description**:
  ...
  - `-binext .ezip`: Specify output file extension as .ezip, can be customized
    to an extension up to 20 characters
...
After program startup, you can dynamically switch image sources through serial
commands:
...
- **LVGL V8**: Use filename directly, e.g., `example.ezip` or
  `/sdcard/image.ezip`
- **LVGL V9**: Need to add drive letter prefix, e.g., `A:/example.ezip` or
  `A:/sdcard/image.ezip`
