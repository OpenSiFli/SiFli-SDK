...
LV_FONT_DECLARE(game_flappy_bird_font_16);
...
static void create_pipeline(lv_timer_t *e)
{
    ...
    if (cnt == 16)
    {
        ...
        lv_img_set_src(area->objs[0], &game_flappy_bird_img_pipeline);
        ...
        lv_img_set_src(area->objs[2], &game_flappy_bird_img_pipeline);
...
static void app_load_cb(void *arg)
{
    ...
    lv_img_set_src(grass, &game_flappy_bird_img_bg_grass);
    lv_obj_align(grass, LV_ALIGN_CENTER, 0, 109);
    lv_obj_set_size(grass, 240, 6);
    timer = lv_timer_create(timer_cb_grass, 16, grass);
    lv_timer_set_repeat_count(timer, -1);

    timer = lv_timer_create(create_pipeline, 300, img);
    lv_timer_set_repeat_count(timer, -1);
